...
%% Retrieve spine size 
soma_spine_contra = get_spine_size(distance_path,soma_spine_contra);
soma_spine_ipsi = get_spine_size(distance_path,soma_spine_ipsi);
soma_spine_binoc = get_spine_size(distance_path,soma_spine_binoc);

%% Retrieve dendritic type
soma_spine_contra = get_dend_type(dendritic_path,soma_spine_contra);
soma_spine_ipsi = get_dend_type(dendritic_path,soma_spine_ipsi);
soma_spine_binoc = get_dend_type(dendritic_path,soma_spine_binoc);

%% Retrieve soma depth
soma_spine_contra = get_soma_type(dendritic_path,soma_spine_contra);
soma_spine_ipsi = get_soma_type(dendritic_path,soma_spine_ipsi);
soma_spine_binoc = get_soma_type(dendritic_path,soma_spine_binoc);
...
%%
 function table_dend = get_spine_size(distance_path,table_dend)
   all_spine_size = [];
   um_to_pixel_sqr = 0.0571^2;
   ...
   for i = 1:height(table_dend)
        ...
        stats_file = fullfile(distance_path, mouse, cells, day, ...
            dend, ['RoiSet', num2str(str2num(split_dend{2})+1),'.csv']);
        if exist(stats_file,"file") & ~isnan(roi)
            tbl = readtable(stats_file);
            area_name = ['Area', num2str(roi)];
            if ~strcmp(tbl.Properties.VariableNames,area_name)
                disp('discrepancy in dendritic distance folder')
               all_spine_size = [all_spine_size; NaN];
            else
                roi_size = (tbl.(area_name))*um_to_pixel_sqr;
                all_spine_size = [all_spine_size; roi_size];
            end
    ...
    table_dend.spine_area = all_spine_size;
end
...
%% retrieve dend_type
function table_dend = get_dend_type(dendritic_path,table_dend)
dend_type = [];
dend_depth = [];
dendritic_file = readtable(dendritic_path);
dendritic_file.mouse_cell = strcat(dendritic_file.("MouseName"), '-', dendritic_file.Cell)
for i = 1:height(table_dend)
    disp(i)
    ...
    mouse_cell = spine.mouse_cell;
    dend_segment = spine.all_fovs;
    dend = split(dend_segment, '_');
    dend = dend{1};
    %get all spines from that mouse,cell, fov
    filter = dendritic_file(contains(dendritic_file.mouse_cell,mouse_cell)...
        & contains(dendritic_file.("FOVName"), dend),:);
    if isempty(filter)
        dend_type = [dend_type; nan];
        dend_depth = [dend_depth; nan];

    else
        dend_type = [dend_type; filter(1,:).("BranchType")];
        dend_depth = [dend_depth; filter(1,:).("Z_axis")];
    end
...
table_dend.dend_type = dend_type;
table_dend.dend_depth = dend_depth;
end

%% retrieve soma depth
function table_dend = get_soma_type(dendritic_path,table_dend)
soma_depth = [];
dendritic_file = readtable(dendritic_path);
dendritic_file.mouse_cell = strcat(dendritic_file.("MouseName"), '-', dendritic_file.Cell)
for i = 1:height(table_dend)
    ...
    %get all spines from that mouse,cell, fov
    filter = dendritic_file(contains(dendritic_file.mouse_cell,mouse_cell)...
        & contains(dendritic_file.("FOVName"), 'Soma'),:);
    if isempty(filter)
        soma_depth = [soma_depth; nan];

    else
        soma_depth = [soma_depth; filter(1,:).("Z_axis")];
    end
...
table_dend.soma_depth = soma_depth;
end
