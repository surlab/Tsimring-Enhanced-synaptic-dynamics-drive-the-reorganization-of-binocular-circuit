...
mouse_path = '/Volumes/GoogleDrive-108846495442099470486/My Drive/Sur Lab/Development project/Binocular_Matching/Spine_imaging/Chronic Imaging/FOV_alignment/to-do';
            ...
            for s = 1:length(unique_FOV)
               if isfolder(fullfile(days(d).folder, days(d).name,...
                   ...
                   for ss = 1:length(unique_seg)
                       in = input(['Do you want to check ' mice(m).name, ' ', day_comp ' ' unique_FOV{s}...
...
%                        end
                       ...
                       unique_FOV{s});
                           ...
                           if exist(fullfile(savepath, ['alignment_based_on_fiducial.mat']),"file")
                                load(fullfile(savepath, ['alignment_based_on_fiducial.mat']), 'modify_mat');
                                modify_mat = runGUI(savepath, fov1_path,fov2_path, roi_file, ref_file, fid1, fid2,modify_mat);
                           else
                        ...
                        save(fullfile(savepath, ['alignment_based_on_fiducial.mat']), 'modify_mat', "-append");
                   end
               end
               end
            end
...
function modified_mat = runGUI(savepath, fov_1_path,fov_2_path, rois_file, ref_file, fid1, fid2, squeeze_matrix)
 ...
 rois1_seg = ReadImageJROI(fullfile(fov_1_path,rois_file));
        ...
        for rr = 1:size(squeeze_matrix,1)
            ...
            in = input(['Do you want to continue: '],'s')
