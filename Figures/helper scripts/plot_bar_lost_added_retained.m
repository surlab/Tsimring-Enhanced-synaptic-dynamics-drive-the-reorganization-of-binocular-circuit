function plot_bar_lost_added_retained(data_feature_not_retained,data_feature_retained, ylim_axis,y_label)
    ...
    plot_errorbar_scatter(mean_data(:,[1,2])',error_data(:,[1,2])', all_data(:,[1,2])')
    [p,a]= ranksum(all_data{1,1}, all_data{2,1})
    plot_sig(1,max(ylim_axis),p);
    [p,a]= ranksum(all_data{1,2}, all_data{2,2})
    plot_sig(2,max(ylim_axis),p);
    set(gca, 'FontSize',10)
    ylim(ylim_axis)
    title(['D1 to D5'])
    ...
    plot_errorbar_scatter(mean_data(:,[3,4])',error_data(:,[3,4])', all_data(:,[3,4])')
