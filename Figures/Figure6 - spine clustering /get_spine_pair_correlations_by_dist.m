...
function [all_mean_soma_resp_corrs_by_session, all_resp_corrs_by_session,all_resp_max_corrs_by_session,all_resps_dists_by_session,...
    all_dists_by_session, all_mouse_cell_by_session,all_unresp_dists_by_session, all_resp_unresp_dists_by_session]...
    =  get_spine_pair_correlations_by_dist(feature,soma_feature,get_soma_resp, all_stim_table, unique_sessions, unique_day, distance_path)
...
for s = 1:length(unique_sessions)
    ...
    all_resp_dists = [];
    all_unresp_dists = [];
    all_resp_unresp_dists = [];
    all_dists = [];
    all_mean_soma_resp_corrs = [];
    ...
    for d = 1:length(unique_day)
        ...
        corr_resp_distances = [];
        all_distances = [];
        all_resp_unresp_distances = [];
        all_unresp_distances = [];
        all_mouse_cell = [];
        max_corr = [];
        ...
        for m = 1:length(unique_mouse_cell)
            ...
            for dd = 1:length(unique_dend)
                ...
                spine_ind = spine_criteria_func(table_spines.resp,table_spines.all_shaft_corr);
                ...
                if exist(stats_file,"file") & ~isnan(table2array(table_spines(1,"all_roi_inds")))
                    ...
                    resp_filter_distances = filter_distances(spine_ind==1, spine_ind==1);
                    unresp_filter_distances =  filter_distances(spine_ind==0, spine_ind==0);
                    resp_unresp_filter_distances =  filter_distances(spine_ind==1, spine_ind==0);
                    ...
                    mask = triu(true(size(corr_spine)),1);
                    all_mask = triu(true(size(filter_distances)),1);
                    
                    corr_activity = [corr_activity; {corr_spine(mask)}];
                    corr_resp_distances = [corr_resp_distances; {resp_filter_distances(mask)}];
                    
                    unresp_mask = triu(true(size(unresp_filter_distances)),1);
                    all_unresp_distances = [all_unresp_distances; {unresp_filter_distances(unresp_mask)}];
                    
                    all_resp_unresp_distances = [all_resp_unresp_distances; {resp_unresp_filter_distances(:)}];
                    
                    mean_soma_resp_corrs = [mean_soma_resp_corrs; {mean_spine_soma(mask)}];
                    all_distances = [all_distances; {filter_distances(all_mask)}];
                    all_mouse_cell = [all_mouse_cell; unique_mouse_cell(m)];

                    corr_spine(1:size(corr_spine, 1)+1:end) = -Inf;
                    max_corr = [max_corr;{max(corr_spine,[],2)}];
                end
            end
        end
        inds = cellfun(@(x) ~isempty(x), corr_activity);
        corr_activity = corr_activity(inds);
        mean_soma_resp_corrs = mean_soma_resp_corrs(inds);
        corr_resp_distances = corr_resp_distances(inds);
        all_max_corr = [all_max_corr; {max_corr}];
        all_distances = all_distances(inds);
        all_mouse_cell = all_mouse_cell(inds);
        all_corrs = [all_corrs, {corr_activity}];
        all_resp_dists = [all_resp_dists, {corr_resp_distances}];
        all_dists = [all_dists, {all_distances}];
        all_mean_soma_resp_corrs = [all_mean_soma_resp_corrs, {mean_soma_resp_corrs}];
        all_mouse_cell_by_day = [all_mouse_cell_by_day, {all_mouse_cell}];
        all_unresp_dists = [all_unresp_dists, {all_unresp_distances}];
        all_resp_unresp_dists = [all_resp_unresp_dists, {all_resp_unresp_distances}];

    end
    all_unresp_dists_by_session.(unique_sessions{s}) = all_unresp_dists;
    all_resp_unresp_dists_by_session.(unique_sessions{s}) = all_resp_unresp_dists;
    all_resp_max_corrs_by_session.(unique_sessions{s}) = all_max_corr;
    all_resp_corrs_by_session.(unique_sessions{s}) = all_corrs;
    all_resps_dists_by_session.(unique_sessions{s}) = all_resp_dists;
    all_dists_by_session.(unique_sessions{s}) = all_dists;
    all_mean_soma_resp_corrs_by_session.(unique_sessions{s}) = all_mean_soma_resp_corrs;
