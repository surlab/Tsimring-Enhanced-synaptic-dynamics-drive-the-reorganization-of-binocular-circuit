function all_counts = get_10day_retained_spine_identity_contra_ipsi_only(retain_D1_D5, retain_D5_D10,mice_cells_D1_D10)
...
for i = 1:length(all_temps)
    ...
    contra_ipsi.cib_d1_to_cib_d5 = strcat(num2str(contra_ipsi.(d1_contra)),num2str(contra_ipsi.(d1_ipsi)),...
        '_',num2str(contra_ipsi.(d5_contra)),num2str(contra_ipsi.(d5_ipsi)));
    
    %label each spine by eye-specific identity for D1 and D5 and count
    %combinations
    a = contra_ipsi.cib_d1_to_cib_d5;
    a = cellstr(a);
